...
public class SignFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		//String server=map.get("server")[0];
		if(map.get("server")!=null&&map.get("server")[0].equals("56846a8a2fee49d14901d39cc48b8b2a")){
			chain.doFilter(request, response);
			return;
		}
		if(map.get("timestamp")==null||map.get("secret")==null||map.get("sign")==null){   //签名参数不能为空
	    	response.reset();
	    ...
	    System.out.println(new Date().getTime());*/
	     if((new Date().getTime()-Long.parseLong(map.get("timestamp")[0]))>10*1000*6){
	    	...
	    	WebApplicationContext wac =   
	    			   WebApplicationContextUtils.getRequiredWebApplicationContext(httpRequest.getSession().getServletContext()); 
