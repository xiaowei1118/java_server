...
public class orderTest {
		...
		try {
			...
			if (togetherIds.size() != 0) {
				for (String togetherId : togetherIds) {
					...
					List<SmallOrder> orderList = orderService.getOrderListInMine(paramMap); // 一单里面的小订单
	...
	public void changeOrderStatus2Buy() {
		...
		if (calendar.get(Calendar.HOUR_OF_DAY) > closeHour
				...
				|| calendar.get(Calendar.HOUR_OF_DAY) < openHour || (calendar.get(Calendar.HOUR_OF_DAY) == openHour
						&& calendar.get(Calendar.MINUTE) < runOpenTime.get(openMinute))) {
		...
		for (String id : orderString) {
			...
			flag = orderService.changeOrderStatus2Buy(phoneId, id, togetherId, rank, reserveTime, message, payWay,
					price, totalPrice);
		...
		if (flag != -1) {
			...
			// 开启线程去访问极光推送
			// pushService.sendPush("18896554880","一笔新的订单已经到达，请前往选单中查看，并尽早分派配送员进行配送。for优。",
			// 1);
			new Thread(new Runnable() {
				...
				public void run() { // 向超级管理员推送，让其分发订单
					...
					// 推送
					// pushService.sendPushByTag("0","一笔新的订单已经到达，请前往选单中查看，并尽早分派配送员进行配送。米奇零点。",
					// 1);
					...
					for (@SuppressWarnings("unused") String phone : superPhones) {
	...
	public void testWebHooks() {
		...
		new Thread(new Runnable() {
			...
			public void run() { // 向超级管理员推送，让其分发订单
				...
				// 推送
				// pushService.sendPushByTag("0","一笔新的订单已经到达，请前往选单中查看，并尽早分派配送员进行配送。For优。",
				// 1);
	...
	public void testRefundOrder() {
		...
		/*
		 * String channel=refund.getChannel(); final String channelString;
		 * if(channel.equals("wx")){ channelString="微信"; }else{
		 * channelString="支付宝"; }
		 */
		@SuppressWarnings("unused")
